$(
    function () {
        ...
        document.getElementById("xianshineirong").innerText = "Browsing content about" + info;
        ...
        $.post("/search", {info: info}, function (result) {
            ...
            while (result.data[k]) {
                var li = document.createElement("li");
                li.innerHTML = "<div class='row youxitupian' id='yingcangtupian_" + k + "'><div class='row'><div class='font font_1'>" + result.data[k].name + "</div></div><div class='row font'><div class='col-md-offset-1'><img class='imgdx' src='/img" + result.data[k].img[1] + "'></div></div><div class='row font font_2'>Tag：</div><div class='row font' id='small_tags_" + k + "'>" + "</div></div>";
                ull.appendChild(li);
                k++;
            }
            i = 0;
