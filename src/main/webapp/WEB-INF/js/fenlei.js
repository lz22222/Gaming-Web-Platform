...
function getgame(page) {
    ...
    $.post(url, {page: page}, function (result) {
        ...
        if(result.data){
            ...
            while (result.data.game[k]) {
                var li = document.createElement("li");
                li.innerHTML = "<div class='row youxitupian' id='yingcangtupian_" + k + "'><div class='row'><div class='font font_1'>" + result.data.game[k].name + "</div></div><div class='row font'><div class='col-md-offset-1'><img class='imgdx' src='/img" + result.data.game[k].img[1] + "'></div></div><div class='row font font_2'>User tag：</div><div class='row font' id='small_tags_" + k + "'>" + "</div></div>";
                ull.appendChild(li);
                k++;
            }
            i = 0;
